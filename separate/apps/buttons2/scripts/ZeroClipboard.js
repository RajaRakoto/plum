...
;(function (window, undefined) {
  ...
  var _containedBy = function (el, ancestorEl) {
    if (
      ...
      el.ownerDocument &&
      ancestorEl &&
      ((ancestorEl.nodeType === 1 &&
        ancestorEl.ownerDocument &&
        ancestorEl.ownerDocument === el.ownerDocument) ||
        (ancestorEl.nodeType === 9 &&
          !ancestorEl.ownerDocument &&
          ancestorEl === el.ownerDocument))
    ) {
      do {
        if (el === ancestorEl) {
          return true
  ...
  var _getCurrentScriptUrlFromError = function () {
    ...
    if (err) {
      url =
        err.sourceURL ||
        err.fileName ||
        _getCurrentScriptUrlFromErrorStack(err.stack)
    }
    return url
  }
  /**
  ...
  var _getDefaultSwfPath = function () {
    var jsDir =
      _getDirPathOfUrl(_getCurrentScriptUrl()) ||
  ...
  var _on = function (eventType, listener) {
    ...
    if (typeof eventType === 'string' && eventType) {
      ...
      for (i in eventType) {
        if (
          _hasOwn.call(eventType, i) &&
          typeof i === 'string' &&
          i &&
          typeof eventType[i] === 'function'
        ) {
  ...
  var _setData = function (format, data) {
    ...
    for (var dataFormat in dataObj) {
      if (
        typeof dataFormat === 'string' &&
        dataFormat &&
        _hasOwn.call(dataObj, dataFormat) &&
  ...
  var _isValidHtml4Id = function (id) {
    return (
      typeof id === 'string' && id && /^[A-Za-z][A-Za-z0-9_:\-\.]*$/.test(id)
    )
  }
  /**
  ...
  var _createEvent = function (event) {
    ...
    if (
      !event.target &&
      /^(copy|aftercopy|_click)$/.test(eventType.toLowerCase())
    ) {
  ...
  var _addMouseData = function (event) {
    if (
      event &&
      /^_(?:click|mouse(?:over|out|down|up|move))$/.test(event.type)
    ) {
      ...
      var fromElement =
        event.type === '_mouseover' && event.relatedTarget
          ...
          : undefined
      var toElement =
        event.type === '_mouseout' && event.relatedTarget
          ? event.relatedTarget
          : undefined
      ...
      var scrollLeft =
        _document.body.scrollLeft + _document.documentElement.scrollLeft
      var scrollTop =
        _document.body.scrollTop + _document.documentElement.scrollTop
      var pageX =
        pos.left + (typeof event._stageX === 'number' ? event._stageX : 0)
      var pageY =
        pos.top + (typeof event._stageY === 'number' ? event._stageY : 0)
  ...
  var _mapClipDataToFlash = function (clipData) {
    ...
    for (var dataFormat in clipData) {
      if (
        dataFormat &&
        _hasOwn.call(clipData, dataFormat) &&
    ...
    for (var prop in clipResults) {
      if (_hasOwn.call(clipResults, prop)) {
        ...
        for (var dataFormat in tmpHash) {
          if (
            dataFormat &&
            _hasOwn.call(tmpHash, dataFormat) &&
            _hasOwn.call(formatMap, dataFormat)
          ) {
  ...
  var _vars = function (options) {
    ...
    if (trustedOriginsExpanded.length) {
      str +=
        'trustedOrigins=' +
        _encodeURIComponent(trustedOriginsExpanded.join(','))
    }
    ...
    if (typeof options.swfObjectId === 'string' && options.swfObjectId) {
      str +=
        (str ? '&' : '') +
        'swfObjectId=' +
        _encodeURIComponent(options.swfObjectId)
    }
    return str
  }
  /**
  ...
  var _detectFlashSupport = function (ActiveXObject) {
    ...
    _flashState.outdated =
      flashVersion &&
      _parseFloat(flashVersion) < _parseFloat(_minimumFlashVersion)
    ...
    for (var i = 0; i < elements.length; i++) {
      if (
        ...
        if (!elements[i].zcClippingId) {
          ...
          _elementMeta[elements[i].zcClippingId].indexOf(this.id) === -1
        ) {
          _elementMeta[elements[i].zcClippingId].push(this.id)
        }
        var clippedElements =
          _clientMeta[this.id] && _clientMeta[this.id].elements
  ...
  var _clientUnclip = function (elements) {
    ...
    for (var i = elements.length; i--; ) {
      if (
        ...
        arrayIndex = 0
        while (
          (arrayIndex = clippedElements.indexOf(elements[i], arrayIndex)) !== -1
        ) {
  ...
  var _clientShouldEmit = function (event) {
    ...
    var goodTarget =
      !event.target ||
      (hasClippedEls && clippedEls.indexOf(event.target) !== -1)
    var goodRelTarget =
      event.relatedTarget &&
      hasClippedEls &&
      clippedEls.indexOf(event.relatedTarget) !== -1
  ...
  var _clientDispatchCallbacks = function (event) {
    ...
    var async = _shouldPerformAsync(event)
    var wildcardTypeHandlers =
      (_clientMeta[this.id] && _clientMeta[this.id].handlers['*']) || []
    var specificTypeHandlers =
